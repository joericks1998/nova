...
class PerformerAttention(tf.Module):
    def __init__(self, embed_dim, num_heads, nb_features, kernel_transformation=None, name=None):
        ...
        self.dense = tf.Variable(tf.random.normal([embed_dim, embed_dim]), name='dense')
    ...
    def __call__(self, q, k, v, mask=None):
        ...
        q = tf.tensordot(q, self.wq, axes=[[1], [0]])  # (batch_size, seq_len, embed_dim)
        k = tf.tensordot(k, self.wk, axes=[[1], [0]])  # (batch_size, seq_len, embed_dim)
        v = tf.tensordot(v, self.wv, axes=[[1], [0]])  # (batch_size, seq_len, embed_dim)
        ...
        output = tf.tensordot(attention_output, self.dense, axes=[[2], [0]])
